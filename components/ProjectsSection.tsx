        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filteredProjects.map((item, index) => (
              <motion.div
                key={item.id}
                className="bg-gray-50 rounded-lg overflow-hidden border border-gray-100 hover:shadow-2xl transition-all duration-300 group perspective-1000"
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  ...
                  />
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
                    initial={{ opacity: 0 }}
                    whileHover={{ opacity: 1 }}
                  >
                    <motion.button
                      onClick={() => navigate(`/project/${item.id}`)}
                      className="bg-white text-brand-blue px-4 py-2 rounded-full font-bold text-sm shadow-lg hover:bg-gray-100 relative overflow-hidden"
                    ...
                    >
                      <motion.div
                        className="absolute inset-0 bg-gradient-to-r from-transparent via-brand-gold/20 to-transparent"
                        initial={{ x: '-100%' }}
                        whileHover={{ x: '100%' }}
                  ...
                  <motion.span
                    className="absolute top-3 right-3 text-xs font-bold bg-white/90 text-brand-blue px-2 py-1 rounded shadow-sm backdrop-blur-sm"
                ...
                <div className="p-4 text-center">
                  ...
                  <motion.div
                    className="h-1 bg-brand-gold mx-auto rounded-full"
                    initial={{ width: 48 }}
                    whileHover={{ width: 96 }}
                    transition={{ duration: 0.3 }}
                  />
